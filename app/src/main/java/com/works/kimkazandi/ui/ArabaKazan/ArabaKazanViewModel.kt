...
class ArabaKazanViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun ArabaKazanfetchData() {
        Thread {
            val isDbEmpty =
                rafflesRoomRepositories.isSelectedCategoryEmpty(Url.arabaKazanSplit) // Uygulama ilk yüklenildiğinde Jsoupdan çekiyoruz
            if (isDbEmpty) {
                val data = rafflesJsoupRepositories.ParseRaffles(Url.arabaKazan)
                for (raffle in data) {
                ...
                val data = rafflesRoomRepositories.getSelectedCategoryRaffles(Url.arabaKazanSplit)
                _list.postValue(data)
