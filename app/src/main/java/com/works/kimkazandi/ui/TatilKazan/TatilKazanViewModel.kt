package com.works.kimkazandi.ui.TatilKazan
...
class TatilKazanViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun TatilKazanfetchData() {
        Thread {
            val isDbEmpty =
                rafflesRoomRepositories.isSelectedCategoryEmpty(Url.tatilKazanSplit) // Uygulama ilk yüklenildiğinde Jsoupdan çekiyoruz
            if (isDbEmpty) {
                val data = rafflesJsoupRepositories.ParseRaffles(Url.tatilKazan)
                for (raffle in data) {
                ...
                val data = rafflesRoomRepositories.getSelectedCategoryRaffles(Url.tatilKazanSplit)
                _list.postValue(data)
